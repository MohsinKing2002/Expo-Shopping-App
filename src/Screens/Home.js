...
const ProductCard = ({ navigation, item, title, img, category, price }) => {
  return (
    <TouchableOpacity
      ...
      className="bg-white w-[45%] border border-gray-200 p-2 rounded-lg m-2"
    >
      <Image
        ...
        source={{ uri: img }}
      />
      <View className="flex items-start pl-1 gap-1">
  ...
  const getProducts = async () => {
    try {
      ...
      if (selectedCat && selectedCat != "all products")
        data = await fetch(
          `${baseUrl}/products/category/${selectedCat}?limit=20`
        );
      else data = await fetch(`${baseUrl}/products?limit=20`);
      ...
      <View className="px-4">
        ...
        <View className="flex flex-row items-center justify-between my-5">
          <View className="w-full bg-gray-100 py-2 px-5 flex flex-row items-center rounded-full">
            ...
            <TextInput
              className="ml-4 text-base text-gray-800 w-full"
